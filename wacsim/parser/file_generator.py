...
class BatchReadmeGenerator:
    ...
    def write_batch(self):
        ...
        with open(str(self.readme_path), 'w') as readme:
            ...
            readme.write(self.get_network_jitter_values())
    ...
    def get_network_jitter_values(self) -> str:
        """Gets the network jitter values of this batch if they exist."""
        if 'network_jitter_values' in self.intermediate_yaml:
            return "\n\n## Network jitter values\n\n{data}" \
                .format(data=str(self.intermediate_yaml['network_jitter_values']))
        else:
            return ""
...
class InputFilesCopier:
    ...
    def copy_input_files(self):
        ...
        if 'network_jitter_data' in self.config:
            copy(self.config_file.parent / self.config['network_jitter_data'],
                 self.configuration_folder / 'network_jitter_data.csv')      
...
class GeneralReadmeGenerator:
    ...
    def get_optional_data_parameters(self) -> str:
        ...
        ret_str += self.checkbox('network_jitter_data')
        ...
        if not self.batch:
            ...
            if 'network_delay_values' in self.intermediate_yaml\
                ...
                ret_str += "\n\n## Network delay values\n\n{data}" \
                    .format(data=str(self.intermediate_yaml['network_delay_values']))
            if 'network_jitter_values' in self.intermediate_yaml\
                    and len(self.intermediate_yaml['network_jitter_values']) > 0:
                ret_str += "\n\n## Network jitter values\n\n{data}" \
                    .format(data=str(self.intermediate_yaml['network_jitter_values']))
