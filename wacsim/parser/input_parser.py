...
class InputParser:
    ...
    def generate_sensor_noise(self):
        """Transforms sensor_noise YAML list into a dict keyed by PLC name.

        Input format (from YAML):
            [{'name': 'PLC1', 'default_noise': 'gaussian', 'default_scale': 0.05,
              'sensors': [{'name': 'T1', 'noise_type': 'uniform', 'scale': 0.03}]}, ...]

        Output format (stored in self.data['sensor_noise']):
            {'PLC1': {'default': {'type': 'gaussian', 'scale': 0.05},
                      'sensors': {'T1': {'type': 'uniform', 'scale': 0.03}}}}
        """
        sensor_noise = {}
        for plc_config in self.data['sensor_noise']:
        ...
        self.data['sensor_noise'] = sensor_noise
