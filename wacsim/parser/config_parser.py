...
from wacsim.parser.input_parser import InputParser
...
class SchemaParser:
        ...
        Or(
                    ...
                    Or('value', 'offset', only_one=True,
                       error="'tags' should have either a 'value' or 'offset' attribute."): Or(
                        ...
                        And(
                            ...
                            error="'value' or 'offset' can be a float or a path to a .csv file"
                        )
                    ),
                }],
            },
            ...
            },
            {
                ...
                'tags': [{
                    ...
                    'scaleParam': And(
                        Or(float, And(int, Use(float))),
                        Schema(lambda i: i != 0, error="'scaleParam' must be different than 0.")
                    ),
                    'scaleTime': And(
                        int,
                        Schema(lambda i: i > 0, error="'scaleTime' must be a positive integer representing iterations.")
                    ),
                }],
            },
            ...
            },
            {
                ...
                'tags': [{
                    ...
                    'noise_type': And(
                        str,
                        Use(str.lower),
                        Or('gaussian', 'gaussian_absolute', 'uniform', 'percentage', 'drift')
                    ),
                    'scale': Or(float, And(int, Use(float))),
                }],
            },
            {
                'type': And(
    ...
    network_events = Schema(
        Or(
            ...
            },
            {
                ...
                Optional('correlation', default=50.0): And(
                    Or(float, And(int, Use(float)))
                )
            }
        )
            ...
            And(
                {
                    ...
                    Optional('noise_scale_data'): And(
                        Use(Path),
                        Use(lambda p: config_path.absolute().parent / p),
                        Schema(lambda l: Path.is_file, error="'noise_scale_data' could not be found."),
                        Schema(lambda f: f.suffix == '.csv',
                               error="Suffix of noise_scale_data should be .csv")),
                    ...
                    Optional('network_jitter_data'): And(
                        Use(Path),
                        Use(lambda p: config_path.absolute().parent / p),
                        Schema(lambda l: Path.is_file, error="'network_jitter_data' could not be found."),
                        Schema(lambda f: f.suffix == '.csv',
                               error="Suffix of network_jitter_data should be .csv")),
                    str: object
                }
            )
        ).validate(data)
    ...
    def validate_schema(data: dict) -> dict:
        ...
        plc_schema = Schema([{
            ...
            Optional(Or('dependent_sensors', 'dependant_sensors', only_one=True,
                       error="Use only one spelling.")): [And(
                str,
                SchemaParser.string_pattern
            )]
        }])

        decision_maker_plc_schema = Schema([{
            'name': And(
                str,
                SchemaParser.string_pattern,
                Schema(lambda name: 1 <= len(name) <= 10,
        ...
        decision_maker_per_scadacommand_schema = Schema([{
            'name': And(
                str,
                SchemaParser.string_pattern,
                Schema(lambda name: 1 <= len(name) <= 10,
        ...
        config_schema = Schema({
            ...
            Optional('sensor_noise'): [{
                'name': And(
                    str,
                    SchemaParser.string_pattern,
                    Schema(lambda name: 1 <= len(name) <= 10,
                           error="Length of PLC name in sensor_noise must be between 1 and 10")),
                Optional('default_noise', default='gaussian'): And(
                    str,
                    Use(str.lower),
                    Or('gaussian', 'gaussian_absolute', 'uniform', 'percentage', 'drift',
                       error="'default_noise' must be one of: gaussian, gaussian_absolute, "
                             "uniform, percentage, drift")),
                Optional('default_scale', default=0.0): Or(float, And(int, Use(float))),
                Optional('sensors'): [{
                    'name': And(str, SchemaParser.string_pattern),
                    'noise_type': And(
                        str,
                        Use(str.lower),
                        Or('gaussian', 'gaussian_absolute', 'uniform', 'percentage', 'drift',
                           error="'noise_type' must be one of: gaussian, gaussian_absolute, "
                                 "uniform, percentage, drift")),
                    'scale': Or(float, And(int, Use(float)))
                }]
            }],
            ...
            Optional('mode', default='plccontrol'): And(
                str,
                Use(str.lower),
                Or('plccontrol', 'scadacontrol', 'hybridcontrol')),

            Optional('decision_maker_per_plc'): decision_maker_plc_schema,
            Optional('decision_maker_per_scadacommand'): decision_maker_per_scadacommand_schema,
...
class ConfigParser:
            ...
            for network_attack in network_attacks:
                ...
                if network_attack['type'] == 'server_mitm':
                    ...
                    for tag in network_attack['tags']:
                        tags.append(tag['tag'])
                    if not set(tags).issubset(set(target_plc['actuators'] 
                                                  + target_plc['sensors']
                                                  + target_plc['dependent_sensors'])):
                        raise NoSuchTag(
    ...
    def generate_intermediate_yaml(self):
        ...
        if 'decision_maker_per_plc' in self.data:
            yaml_data['decision_maker_per_plc'] = self.data['decision_maker_per_plc']
        else:
            yaml_data['decision_maker_per_plc'] = []
        if 'decision_maker_per_scadacommand' in self.data:
            yaml_data['decision_maker_per_scadacommand'] = self.data['decision_maker_per_scadacommand']
        else:
            yaml_data['decision_maker_per_scadacommand'] = []
        ...
        if 'network_jitter_data' in self.data:
            yaml_data['network_jitter_data'] = str(self.data['network_jitter_data'])
        if 'noise_scale_data' in self.data:
            yaml_data['noise_scale_data'] = str(self.data['noise_scale_data'])
        if 'sensor_noise' in self.data:
            yaml_data['sensor_noise'] = self.data['sensor_noise']
