...
class NoiseInjectionAttack(MiTMAttack):
    """
    Noise injection attack orchestrator. Overrides setup to spawn the
    noise_injection_netfilter_queue.py subprocess.
    """
    def setup(self):
        ...
        self.logger.debug(f"Noise Injection Attack ARP Poison between {self.target_plc_ip} and "
                          f"{self.intermediate_attack['gateway_ip']}")

        queue_number = self.intermediate_attack['queue_num']
        nfqueue_path = Path(__file__).parent.absolute() / "noise_injection_netfilter_queue.py"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Start Noise Injection attack')
