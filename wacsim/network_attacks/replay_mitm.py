...
class ReplayMiTMAttack(SyncedAttack):
    ...
    def modify_ip_tables(self, append=True):
        ...
        if append:
            ...
            os.system(f'iptables -w -t mangle -D INPUT -p tcp -j NFQUEUE --queue-num {queue_number}')
            os.system(f'iptables -w -t mangle -D FORWARD -p tcp -j NFQUEUE --queue-num {queue_number}')
