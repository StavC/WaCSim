...
class EventControl(NodeControl):
    ...
    def start_event(self):
        ...
        elif self.this_event_data['type'] == 'link_down':
            generic_event = Path(__file__).parent.parent.absolute() / "network_events" / "link_down.py"
        elif self.this_event_data['type'] == 'rate_limit':
            generic_event = Path(__file__).parent.parent.absolute() / "network_events" / "rate_limit.py"
        elif self.this_event_data['type'] == 'bursty_loss':
            generic_event = Path(__file__).parent.parent.absolute() / "network_events" / "bursty_loss.py"
        elif self.this_event_data['type'] == 'ip_conflict':
            generic_event = Path(__file__).parent.parent.absolute() / "network_events" / "ip_conflict.py"
        elif self.this_event_data['type'] == 'packet_reorder':
            generic_event = Path(__file__).parent.parent.absolute() / "network_events" / "packet_reorder.py"
        else:
