...
class SimpleTopo(Topo):
    ...
    def add_node_switch_link(self, node, switch, yaml_node_data):
        ...
        if 'network_jitter_data' in self.data and\
                self.data['network_jitter_values'][yaml_node_data['name']]:
            link_params['jitter'] = self.data['network_jitter_values'][yaml_node_data['name']]
    ...
    def setup_network(self, net):
        ...
        if 'plcs' in self.data.keys():
            for plc in self.data['plcs']:
                node = net.get(plc['name'])
                node.cmd('route add default gw {ip}'.format(ip=plc['gateway']))
                node.cmd('sysctl -w net.ipv4.conf.all.accept_redirects=1')
                node.cmd('sysctl -w net.ipv4.conf.default.accept_redirects=1')
                node.cmd('sysctl -w net.ipv4.conf.all.secure_redirects=0')
                node.cmd('sysctl -w net.ipv4.conf.default.secure_redirects=0')
                node.cmd('sysctl -w net.ipv4.conf.{intf}.accept_redirects=1'.format(intf=plc['interface']))
                node.cmd('sysctl -w net.ipv4.conf.{intf}.secure_redirects=0'.format(intf=plc['interface']))

        # Set the default gateway of the SCADA
        scada_node = net.get('scada')
        scada_node.cmd('route add default gw ' + self.supervisor_ip)
        scada_node.cmd('sysctl -w net.ipv4.conf.all.accept_redirects=1')
        scada_node.cmd('sysctl -w net.ipv4.conf.default.accept_redirects=1')
        scada_node.cmd('sysctl -w net.ipv4.conf.all.secure_redirects=0')
        scada_node.cmd('sysctl -w net.ipv4.conf.default.secure_redirects=0')
        scada_node.cmd('sysctl -w net.ipv4.conf.{intf}.accept_redirects=1'.format(intf=self.data['scada']['interface']))
        scada_node.cmd('sysctl -w net.ipv4.conf.{intf}.secure_redirects=0'.format(intf=self.data['scada']['interface']))
