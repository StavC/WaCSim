...
**type** (required for network attacks)
  One of: ``simple_dos``, ``naive_mitm``, ``mitm``, ``server_mitm``, ``seq_mitm``, ``replay_mitm``, ``concealment_mitm``, ``noise_injection``, ``rogue_scada``, ``tcp_rst``, ``icmp_redirect_mitm``. See below for a short description and minimal example for each.
...
**Which attacks support it:** The **``mitm``** and **``rogue_scada``** attack types support using a **path to a CSV file** for ``value`` or ``offset``. The other MitM types use **numeric** value/offset only:

- **``mitm``**, **``rogue_scada``** — CSV path supported for ``value`` and ``offset`` per tag (time-varying sequence).
...
- **``noise_injection``** — Does not use ``value``/``offset`` (it applies noise via ``noise_type`` and ``scale``).
...
**noise_injection**
  Intercepts sensor readings and adds mathematical noise to them. You list the tags to target under **``tags``**, and for each tag you specify:

  - **``tag``** (required) — Tag name (e.g. ``T1``).
  - **``noise_type``** (required) — One of:
    - ``gaussian``: Relative gaussian noise ($value + \mathcal{N}(0, scale \times value)$).
    - ``gaussian_absolute``: Absolute gaussian noise ($value + \mathcal{N}(0, scale)$).
    - ``uniform``: Relative uniform noise ($value + \mathcal{U}(-scale \times value, scale \times value)$).
    - ``percentage``: Uniform percentage scaling ($value \times (1 + \mathcal{U}(-scale, scale))$).
    - ``drift``: Incremental drift over time ($value + scale \times iteration$).
  - **``scale``** (required) — Float multiplier defining the noise scale or drift rate.
...
**rogue_scada**
  Simulates a compromised/rogue SCADA connection by sending unauthorized control commands directly to a target PLC, or intercepting command writes over the wire and overriding them. You list which tags (typically SCADA commands) to change under **``tags``**, each with:

  - **``tag``** (required) — The command tag to override (e.g. ``ScadaCommand_P1``).
  - **``value``** or **``offset``** (required) — The override value (or offset to add). Can be a number (float/int) or a **path to a CSV file** for time-varying command sequences (e.g. ``rogue_values_P1.csv``).
...
**tcp_rst**
  Performs TCP Reset (RST) Injection on EtherNet/IP (port 44818) connections. The attacker intercepts TCP packets sent between the target PLC and the SCADA or peer PLCs, extracts sequence and acknowledgment numbers, and injects forged TCP packets with the ``RST`` flag set. This forces the recipient node to immediately tear down the active socket connection, causing communication loss and forcing the nodes to attempt socket reconnection.
...
**icmp_redirect_mitm**
  Performs an ICMP Redirect Man-in-the-Middle attack. The attacker spoof-sends ICMP Redirect (Type 5, Code 1) packets to the target PLC, pretending to be its default gateway. The packets redirect the target PLC's SCADA-bound and peer-bound TCP traffic through the attacker node. The attacker runs a netfilter queue process to intercept the redirected TCP packets and dynamically modify the payload of the response packets containing the specified tag values or offsets.
