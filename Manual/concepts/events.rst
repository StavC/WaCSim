...
**link_down**
  Simulates a total communication blackout (100% packet loss) on the link. Requires no extra parameters besides the common options. Useful for emulating physical link failure (e.g., fiber cut).

**rate_limit**
  Restricts the communication bandwidth/throughput of the link. **``value``** specifies the bandwidth limit in **kilobits per second (kbps)**. For example, ``value: 10`` limits throughput to 10 kbps, simulating severe congestion. Implemented via Linux netem rate limiting.

**bursty_loss**
  Simulates bursty packet drops using a 2-state Gilbert-Elliott Markov chain model. Requires **``state_g2b``** (Good-to-Bad transition probability, in %) and **``state_b2g``** (Bad-to-Good transition probability, in %). Perfect for emulating fading or interference on remote wireless/radio channels.

**ip_conflict**
  Simulates an IP address collision by toggling the link state between 100% loss (blackout) and 0% loss (connected) on alternate iterations. Requires no extra parameters besides common options. Models competing ARP responses.

**packet_reorder**
  Delays packets while delivering a percentage of them out of sequence. Requires **``delay_value``** (milliseconds) and **``percent``** (percentage of packets to deliver out of sequence). Optional **``correlation``** (percent correlation factor, defaults to 50%). Emulates multi-path routing.
...
**target** (required)
  The **PLC name** (e.g. ``PLC1``, ``PLC4``) or **``scada``** that this event targets. Delay/loss/jitter are applied on the link to that node.
...
**value** (required for ``packet_loss``, ``network_delay``, and ``rate_limit``)
  - For **packet_loss**: loss rate in percent (0–100).
  - For **network_delay**: delay in milliseconds.
  - For **rate_limit**: bandwidth in kbps.
...
**state_g2b** and **state_b2g** (required for ``bursty_loss``)
  - Good-to-Bad transition probability (%) and Bad-to-Good transition probability (%), respectively.

**delay_value**, **percent**, and **correlation** (required for ``packet_reorder``)
  - **delay_value**: delay in milliseconds.
  - **percent**: percentage of packets to deliver out of sequence (0–100).
  - **correlation** (optional): correlation factor in percent (0–100, defaults to 50).
...
See ``Manual/demo_files/demo_events.yaml`` for a comprehensive events template. Below are some illustrative examples:

**Link Blackout (100% loss) when Tank T1 is low:**
...
**Bursty Wireless Loss (Gilbert-Elliott) for 50 iterations:**
...
**Packet Reordering (out-of-sequence delivery) when Tank T2 is high:**
