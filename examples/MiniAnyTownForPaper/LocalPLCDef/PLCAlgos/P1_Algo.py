...
def CheckForTeardown(csv_content, noise_threshold=0.1):
    ...
    J2_value = csv_content['J2'].iloc[-1:].values

    # Check if the current value differs significantly from both previous values
    if J2_value[0] <=50:
        print("Teardown condition met: Less than 50 Pressure in J2, returning to rule")
        return True
...
def CheckForFlowDrop(csv_content):
    ...
    if 'P1F' not in csv_content or len(csv_content['P1F']) < 2:
...
def AlgoRun(cacheDict,LocalSensorsValues):
    STATE_FILE = "state.txt"  # File to record the current state
    ...
    PLCNAME='PLC4'
    prefix = 'examples/MiniAnyTownForPaper/LocalPLCDef/PLCAlgos/'
    ...
    """
    J2	P1F	V_UF	P1	T41
0	0	0	1	3.05100011825562
69.1993172472623	213.393503019295	233.338957218621	1	3.19157791137695
69.2178528705034	212.416547842605	232.284307747932	1	3.3316547870636
69.2178528705034	212.416547842605	232.284307747932	1	3.3316547870636
69.2363559395635	211.441308495763	231.230533264579	1	3.47122406959534
69.2548255611276	210.467832062636	230.177697509362	1	3.61028003692627
"""
