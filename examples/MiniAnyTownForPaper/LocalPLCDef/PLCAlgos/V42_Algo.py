...
def AlgoRun(cacheDict,LocalSensorsValues):
    ...
    if csv_content['T42'].iloc[-1:].values[0] >=10.0:
        if csv_content['J19'].iloc[-1:].values[0] <=56.0:
            print(f'J19 value is less than 50, meaning that the pump is off hence we can open the valve')
            return 'open',True
        else:
