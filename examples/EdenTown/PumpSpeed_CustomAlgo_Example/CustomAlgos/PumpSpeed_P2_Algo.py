...
def AlgoRun(plc_cache, plc_dict, scada_cache=None, control=None):
    ...
    # Get tank T2 level from local sensors
    # The tank level is stored with key ('T2', 1)
    tank_level = None
    
    # Try to get tank level from plc_dict (local sensors)
    if ('T2', 1) in plc_dict:
        tank_level = plc_dict[('T2', 1)]
    # Try from plc_cache if not found locally
    elif plc_cache is not None and 'T2' in plc_cache.columns:
        try:
            tank_level = plc_cache['T2'].iloc[-1]  # Get latest value
        except:
            pass
